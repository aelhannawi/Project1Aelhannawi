...
def create_top250_movies_table(cursor: sqlite3.Cursor):
...
def create_mostPopular_TV_table(cursor: sqlite3.Cursor):
    cursor.execute('''CREATE TABLE IF NOT EXISTS most_popularTV_data(
        ttid TEXT PRIMARY KEY,
        rank INTEGER DEFAULT 0,
...
def create_mostPopular_movies_table(cursor: sqlite3.Cursor):
    cursor.execute('''CREATE TABLE IF NOT EXISTS most_popular_movies_data(
            ttid TEXT PRIMARY KEY,
            rank INTEGER DEFAULT 0,
            rankUpDown INTEGER DEFAULT 0,
            title TEXT,
...
def create_UpDown_ranking_table(cursor: sqlite3.Cursor):
    cursor.execute('''CREATE TABLE IF NOT EXISTS updown_ranking_data(
            ttid TEXT PRIMARY KEY,
            rank INTEGER DEFAULT 0,
            rankUpDown INTEGER DEFAULT 0,
            title TEXT,
...
def put_top_movies_in_database(data_to_add: list[tuple], db_cursor: sqlite3.Cursor):
    db_cursor.executemany("""INSERT INTO top_movies_data(ttid, rank, title, fulltitle, year, image_url, crew, imdb_rating,
    imdb_rating_count)
    VALUES(?,?,?,?,?,?,?,?,?)""", data_to_add)


def put_most_PopularTV_in_database(data_to_add: list[tuple], db_cursor: sqlite3.Cursor):
    db_cursor.executemany("""INSERT INTO most_popularTV_data(ttid, rank,rankUpDown, title, fulltitle, year, image_url, crew, imdb_rating,
    imdb_rating_count)
    VALUES(?,?,?,?,?,?,?,?,?)""", data_to_add)


def put_most_Popular_movies_in_database(data_to_add: list[tuple], db_cursor: sqlite3.Cursor):
    db_cursor.executemany("""INSERT INTO most_popular_movies_data(ttid, rank,rankUpDown, title, fulltitle, year, image_url, crew, imdb_rating,
        imdb_rating_count)
        VALUES(?,?,?,?,?,?,?,?,?)""", data_to_add)

def put_UpDown_ranking_in_database(data_to_add: list[tuple], db_cursor: sqlite3.Cursor):
    db_cursor.executemany("""INSERT INTO updown_ranking_data (ttid, rank,rankUpDown, title, fulltitle, year, image_url, crew, imdb_rating,
           imdb_rating_count)
           VALUES(?,?,?,?,?,?,?,?,?)""", data_to_add)
